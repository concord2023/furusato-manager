...
function renderHome(){const pack=window.FurusatoGoogleDrive?.buildYearCalculationState?.(state,Number(state.year)||new Date().getFullYear());const c=pack?.result||calc();for(const [id,v] of [['limit',yen(c.estimatedLimit)],['donated',yen(c.donated)],['remaining',yen(c.remaining)],['gross',yen(c.employmentGross)],['social',yen(c.social)]]){const e=document.getElementById(id);if(e)e.textContent=v}const p=(state.donations||[]).filter(x=>x.status==='確認待ち').length;const e=document.getElementById('pendingCount');if(e)e.textContent=p+'件'}
...
function resetSample(){['furusatoState','furusatoStateBackup','furusatoImportHistory','furusatoPayrollStore'].forEach(k=>localStorage.removeItem(k));location.href='index.html'}
