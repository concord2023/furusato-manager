...
const FurusatoGoogleDrive = (() => {
  ...
  function classifyPdfText(text,name=''){
    const t=`${name}\n${text}`;if(/bonus|賞与|ボーナス/i.test(name))return 'bonus_slip';if(/gensen|源泉徴収|withholding/i.test(name))return 'withholding';if(/chinginmeisai|給与|給料|賃金|salary/i.test(name))return 'salary_slip';if(/賞与|ボーナス|bonus/i.test(t))return 'bonus_slip';if(/源泉徴収票|給与所得の源泉徴収票|withholding/i.test(t))return 'withholding';if(/給与|給料|salary|支給合計/i.test(t))return 'salary_slip';return 'unknown'
  }
