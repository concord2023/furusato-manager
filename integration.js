...
const FurusatoGoogleDrive = (() => {
  ...
  function parseSalaryPdf(text,name){const monthMatch=text.match(/(20\d{2})年\s*(\d{1,2})月|20\d{2}[-/](\d{1,2})/);const year=monthMatch?Number(monthMatch[1]||monthMatch[0].slice(0,4)):null;const month=monthMatch?Number(monthMatch[2]||monthMatch[3]):null;const gross=extractNumberAfter(text,[/(?:支給合計|総支給額|支給額|Total Payment)\s*[^\d]{0,20}([\d,]{4,})/i]);return {year,month,gross,source:'google-drive',status:'actual',document:name,needsReview:!(month&&gross)} }
  function parseBonusPdf(text,name){const amount=extractNumberAfter(text,[/(?:支給合計|総支給額|支給額|Total Payment)\s*[^\d]{0,20}([\d,]{5,})/i]);const date=text.match(/(20\d{2})年\s*(\d{1,2})月|20\d{2}[-/](\d{1,2})/);return {date:date?`${date[1]||date[0].slice(0,4)}-${String(date[2]||date[3]).padStart(2,'0')}`:'',amount,source:'google-drive',status:'actual',document:name,needsReview:!amount}}
  async function scanAndImport(state,onProgress){const files=await listCandidateFiles();const result={files:files.length,added:0,review:0,skipped:0,errors:[]};for(const f of files){try{onProgress?.(`解析中: ${f.name}`);const buf=await downloadPdf(f.id);const text=await pdfText(buf);const type=classifyPdfText(text,f.name);if(type==='salary_slip'){const x=parseSalaryPdf(text,f.name);if(x.year===state.year&&x.month&&x.gross){const existing=(state.salaryRecords||[]).find(r=>Number(r.month)===x.month&&Number(r.gross)===x.gross);if(!existing){state.salaryRecords=(state.salaryRecords||[]).filter(r=>Number(r.month)!==x.month);state.salaryRecords.push({month:x.month,gross:x.gross,source:'google-drive',status:'actual',document:f.name,driveFileId:f.id});result.added++}}else result.review++}else if(type==='bonus_slip'){const x=parseBonusPdf(text,f.name);if(x.amount){const existing=(state.bonusRecords||[]).some(r=>r.document===f.name||Number(r.amount)===Number(x.amount)&&r.date===x.date);if(!existing){state.bonusRecords.push({...x,driveFileId:f.id});result.added++}}else result.review++}else if(type==='withholding'){state.sourceDocuments=state.sourceDocuments||[];if(!state.sourceDocuments.some(x=>x.file===f.name))state.sourceDocuments.push({file:f.name,type:type,status:'imported',source:'google-drive',driveFileId:f.id,note:'源泉徴収票を取得。詳細項目の抽出は確認待ち。'});result.review++}else result.skipped++}catch(e){result.errors.push(`${f.name}: ${e.message}`)}}return result}
