...
const FurusatoGoogleDrive = (() => {
  ...
  async function pdfText(arrayBuffer){
    if(!window.pdfjsLib){
      // PDF.js 3.11 is too old for some unembedded Japanese CID fonts used by
      // Toyota's JasperReports/iText PDFs. Use the modern legacy ESM build;
      // it has newer font/CMap handling while retaining the same API.
    ...
    if(pdf.GlobalWorkerOptions)pdf.GlobalWorkerOptions.workerSrc='https://cdn.jsdelivr.net/npm/pdfjs-dist@4.10.38/legacy/build/pdf.worker.min.mjs';
  ...
  function parseSalaryPdf(text,name){
    ...
    if(text&&typeof text==='object'&&text.ocrText){
      ...
      c={...c,unknownComponents:[...(c.grossTotal==null?['支給合計']:[]),...(c.nonTaxableTotal==null?['非課税分']:[]),...(c.taxableGross==null?['課税対象額']:[]),...(c.socialTotal==null?['社会保険料']:[])],arithmeticOk:derived!=null&&c.taxableGross===derived&&socialOk,needsReview:!(derived!=null&&c.taxableGross===derived&&socialOk)};
    }
