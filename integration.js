...
const FurusatoGoogleDrive = (() => {
  ...
  function upsertSalaryRecord(state,x,f,targetYear,result){
    ...
    archiveRow(targetYear,'salaryRecords',state.salaryRecords.find(r=>Number(r.month)===Number(x.month)&&r.source==='google-drive')||{year:targetYear,month:x.month,gross:x.taxableGross,taxableGross:x.taxableGross,grossTotal:x.grossTotal,nonTaxableTotal:x.nonTaxableTotal,components:x.components||[],source:'google-drive',status:'actual',document:f.name,driveFileId:f.id,needsReview:x.needsReview},r=>`${r.month}|${r.document||r.driveFileId||''}`); if(x.social!=null)archiveRow(targetYear,'socialRecords',state.socialRecords.find(r=>Number(r.month)===Number(x.month)&&r.source==='google-drive')||{year:targetYear,month:x.month,amount:x.social,components:x.socialComponents||{},source:'google-drive',status:'actual',document:f.name,driveFileId:f.id},r=>`${r.month}|${r.document||r.driveFileId||''}`); if(x.needsReview){result.review++;result.salaryReview++}result.added++;result.salaryImported++; return true;
  }
  function upsertBonusRecord(state,x,f,targetYear,result){
    ...
    archiveRow(targetYear,'bonusRecords',state.bonusRecords.find(r=>Number(r.year||String(r.date||'').slice(0,4))===Number(targetYear)&&r.season===season&&r.source==='google-drive')||x,r=>`${r.date||''}|${r.season||''}|${r.document||r.driveFileId||''}`); result.added++;return true;
  }
  async function scanAndImport(state,onProgress,filesOverride){
    ...
    const archiveYear=y=>{const k=String(Number(y));const a=state.yearPayrollRecords[k]&&typeof state.yearPayrollRecords[k]==='object'?state.yearPayrollRecords[k]:{year:Number(y),salaryRecords:[],socialRecords:[],bonusRecords:[]};a.year=Number(y);a.salaryRecords=Array.isArray(a.salaryRecords)?a.salaryRecords:[];a.socialRecords=Array.isArray(a.socialRecords)?a.socialRecords:[];a.bonusRecords=Array.isArray(a.bonusRecords)?a.bonusRecords:[];state.yearPayrollRecords[k]=a;return a};
    const archiveRow=(y,type,row,key)=>{if(!row)return;const a=archiveYear(y),arr=a[type];const i=arr.findIndex(x=>key(x)===key(row));if(i>=0)arr[i]={...arr[i],...FurusatoModel.clone(row)};else arr.push(FurusatoModel.clone(row));};
    // Copy both target and prior rows into the permanent year archive before this scan changes live buffers.
    for(const r of state.salaryRecords||[])if(Number(r.year))archiveRow(r.year,'salaryRecords',r,x=>`${x.month}|${x.document||x.driveFileId||''}`);
    for(const r of state.socialRecords||[])if(Number(r.year))archiveRow(r.year,'socialRecords',r,x=>`${x.month}|${x.document||x.driveFileId||''}`);
    for(const r of state.bonusRecords||[])if(Number(r.year||String(r.date||'').slice(0,4)))archiveRow(r.year||String(r.date||'').slice(0,4),'bonusRecords',r,x=>`${x.date||''}|${x.season||''}|${x.document||x.driveFileId||''}`);
    for(const r of state.priorSalaryRecords||[])if(Number(r.year))archiveRow(r.year,'salaryRecords',r,x=>`${x.month}|${x.document||x.driveFileId||''}`);
    for(const r of state.priorSocialRecords||[])if(Number(r.year))archiveRow(r.year,'socialRecords',r,x=>`${x.month}|${x.document||x.driveFileId||''}`);
    for(const r of state.priorBonusRecords||[])if(Number(r.year))archiveRow(r.year,'bonusRecords',r,x=>`${x.date||''}|${x.season||''}|${x.document||x.driveFileId||''}`);
