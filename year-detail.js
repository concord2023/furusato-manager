  ...
  document.addEventListener('DOMContentLoaded',()=>{
    ...
    function render(){
      ...
      $('calculationBreakdown').innerHTML=x?`
        ...
        <div class="row"><span>課税対象の追加調整</span><strong>${yen(x.adjustmentTotal)}</strong></div><div class="row"><span>特別支給・補助（課税・手動）</span><strong>${yen(x.tempTaxable??x.temporary)}</strong></div>
      ...
      const manual=r.manual||{}; const life=manual.lifeInsurance||{}, eq=manual.earthquakeDetail||{};
      $('pension').value=Number(manual.nationalPension)||0; $('temporary').value=Number(manual.temporary)||0; $('ideco').value=Number(manual.ideco)||0;
      $('lifeNew').value=Number(life.newGeneral)||0; $('lifeOld').value=Number(life.oldGeneral)||0; $('lifeMedical').value=Number(life.nursingMedical)||0; $('lifePensionNew').value=Number(life.newPension)||0; $('lifePensionOld').value=Number(life.oldPension)||0; $('eqPaid').value=Number(eq.paid)||0; $('eqOld').value=Number(eq.oldLongTerm)||0;
    ...
    $('save').onclick=()=>{const n=Math.max(0,Number($('pension').value)||0), temporary=Math.max(0,Number($('temporary').value)||0), ideco=Math.max(0,Number($('ideco').value)||0);state.yearRecords=state.yearRecords&&typeof state.yearRecords==='object'?state.yearRecords:{};const prev=state.yearRecords[String(y)]||{year:y,manual:{}};state.yearRecords[String(y)]={...prev,year:y,manual:{...(prev.manual||{}),nationalPension:n,temporary,temporaryTaxable:true,ideco,lifeInsurance:{newGeneral:Math.max(0,Number($('lifeNew').value)||0),oldGeneral:Math.max(0,Number($('lifeOld').value)||0),nursingMedical:Math.max(0,Number($('lifeMedical').value)||0),newPension:Math.max(0,Number($('lifePensionNew').value)||0),oldPension:Math.max(0,Number($('lifePensionOld').value)||0)},earthquakeDetail:{paid:Math.max(0,Number($('eqPaid').value)||0),oldLongTerm:Math.max(0,Number($('eqOld').value)||0)},lifeInsuranceOverride:[Number($('lifeNew').value)||0,Number($('lifeOld').value)||0,Number($('lifeMedical').value)||0,Number($('lifePensionNew').value)||0,Number($('lifePensionOld').value)||0].some(v=>v>0),earthquakeOverride:[Number($('eqPaid').value)||0,Number($('eqOld').value)||0].some(v=>v>0)},updatedAt:new Date().toISOString()};save();render()};
